...
function toggleTrip(button) {
    ...
    const hiddenInput = row.querySelector('.is-booked');
    const bookAllBtn = document.getElementById('book-all-btn');
    ...
    if (button.classList.contains('selected')) {
        ...
        window.selectedTrips = window.selectedTrips.filter(trip => 
            trip.carNumber !== tripData.carNumber || trip.time !== tripData.time
        );
    ...
    // Enable/disable book all button based on selections
...
function updateSelectedTripsUI() {
    ...
    window.selectedTrips.forEach(trip => {
        ...
        tripElement.classList.add('selected-trip-item');
        tripElement.innerHTML = `
            <span>${trip.carNumber} - ${trip.destination}</span>
            <span>${trip.time} - $${trip.price}</span>
        `;
        selectedTripsList.appendChild(tripElement);
    });

    // Enable/disable book all button
    bookAllBtn.disabled = window.selectedTrips.length === 0;
}
