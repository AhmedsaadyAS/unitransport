...
namespace UniTransport.PLL.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> DeleteTrip(int tripId)
        {
            ...
            {
                _logger.LogError(ex, "Database error while deleting trip {TripId}", tripId);
                return Json(new { success = false, message = "Database error occurred while deleting the trip. The trip may have related records." });
            }
            catch (Exception ex)
            {
